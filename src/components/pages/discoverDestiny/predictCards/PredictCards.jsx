...
const PredictCards = ({ togglePredictDetails, showPredictDetails, selectedCards }) => {
  ...
  return (
    <div className="relative pb-[70px] w-full h-fit">
      <div
        className={`grid grid-cols-3 gap-y-3 justify-items-center lg:flex justify-center  gap-x-3 ${cardOpacity} duration-700 w-full`}
      >
        {genarateCards.map((index) => (
          <Image
            ...
            width={100}
            height={100}
            className="w-[80px] h-[100px] lg:w-[40px] lg:h-[60px]"
          ></Image>
        ))}
      </div>
      {
