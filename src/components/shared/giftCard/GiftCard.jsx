...
const GiftCard = ({ home }) => {
  ...
  return (
    <div
      className={`p-5 ${
        home
          ? "mt-4 w-full lg:h-[90%] 2xl:w-full rounded-xl"
      ...
      <button
        ...
        className={`text-xs w-full px-2 py-3 font-berlin text-[#E5BD9D] absolute left-1/2 top-1/3 -translate-x-1/2 -translate-y-1/3 duration-500 ${
          takeCardIsClicked ? "opacity-100" : "opacity-0"
        }`}>
        <p>You received one coupon</p>
      </button>
      <div
        className={`flex flex-col w-full h-full items-center justify-between duration-500 ${
